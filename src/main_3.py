...
from pre_process_data import MultiOmicsData, get_mask, get_mask_wrapper
from train_eval import create_optimizer, model_train_3
...
def run_model(
    ...
    mask_train = get_mask(
        3, data["meth"][dataset.graph.idx_dict["tr"]].shape[0], config["missing_rate"]
    )
    ...
    for idx, (k, v) in enumerate(data_clone_tr.items()):
        ...
        v[dataset.graph.idx_dict["tr"]] = (
            v[dataset.graph.idx_dict["tr"]] * masked_dim_train
        )
        data_train[k] = v
    mask_train = mask_train
    mask_test = get_mask_wrapper(
        3,
        data_clone_te["meth"][dataset.graph.idx_dict["te"]].shape[0],
        config["missing_rate"],
    )
    ...
    for idx, (k, v) in enumerate(data_clone_te.items()):
        ...
        v[dataset.graph.idx_dict["te"]] = (
            v[dataset.graph.idx_dict["te"]] * masked_dim_test
        )
        data_test[k] = v
    mask_test = mask_test
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        if res1 > global_acc:
            global_acc = res1
            if dataset.graph.num_class == 2:
                best_eval = {"acc": res1, "f1": res2, "auc": res3}
            else:
                best_eval = {"acc": res1, "f1w": res2, "f1m": res3}
