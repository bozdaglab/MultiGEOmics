...
class MultiOmicsData(DGLDataset):
    ...
    def process(self):
        ...
        if "idx_dict" in train_test_data.keys():
            self.graph.idx_dict = train_test_data["idx_dict"]
            del train_test_data["idx_dict"]
        try:
            for omic in self.omics_type:
...
def define_dataset(folder_name: str) -> List[str]:
    if folder_name == DataEnum.TCGA_BRCA.name:
        ...
        omics_type = WT
    elif folder_name == DataEnum.ROSMAP_M.name:
        omics_type = ROSMAP_M
    elif folder_name == DataEnum.BRCA_M.name:
        omics_type = BRCA_M
    elif folder_name == DataEnum.LGG.name:
        omics_type = LGG
    elif folder_name == DataEnum.KIPAN.name:
        omics_type = KIPAN
    else:
        raise ValueError
    return omics_type
...
def create_mrr_dataset(folder_name):
    ...
    data_name_copy = data_name.copy()
    for idx in range(1, len(data_name_copy)):
        data_name.append(f"{data_name_copy[idx - 1]}_{data_name_copy[idx]}")
    data_name_copy.reverse()
    for idx in range(1, len(data_name_copy)):
        data_name.append(f"{data_name_copy[idx - 1]}_{data_name_copy[idx]}")
    return data_name
